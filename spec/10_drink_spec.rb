...
describe Drink do
  describe '#initialize' do
    context 'when type is specified and ounces is default' do
      ...
      # beverage type.
      subject(:fresca) { described_class.new('soda')}

      # remove the 'x' before running this test
      it 'is your choice of beverage' do
        expect(fresca.type).to eq('soda')
      end

      # remove the 'x' before running this test
      it 'has 16 ounces' do
        expect(fresca.ounces).to eq(16)
      end
    end
  end
  ...
  describe '#full?' do
    ...
    context 'when drink has less than 16 ounces' do
      ...
      subject(:fresca) { described_class.new('soda', 8)}
      # remove the 'x' before running this test
      it 'is not full' do
        expect(fresca).not_to be_full
      end
    end
  end
end
